...
func (d Data) ProgressBar(ctx context.Context) error {
	...
	for {
		...
		default:
			...
			if err != nil {
				return errors.New("failed to get directory size: " + err.Error())
			}
...
func (d *Data) BindwithFiles(procs int) error {
	...
	if err != nil {
		return errors.New("failed to create a file in download location: " + err.Error())
	}
	defer fh.Close()
	...
	for i := 0; i < procs; i++ {
		...
		subfp, err := os.Open(f)
		if err != nil {
			return errors.New("failed to open " + f + " in download location: " + err.Error())
		}
		...
		if err := os.Remove(f); err != nil {
			return errors.New("failed to remove a file in download location: " + err.Error())
		}
	}
	...
	if err := os.RemoveAll(dirname); err != nil {
		return errors.New("failed to remove download location: " + err.Error())
	}
